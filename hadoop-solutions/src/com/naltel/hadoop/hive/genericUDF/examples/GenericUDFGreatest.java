 ...
 * Oracle's CREATEST returns the greatest of the list of one or more
 * expressions.
 * 
 */
@Description(name = "greatest", value = "_FUNC_(value1, value2, value3, ....) "
		+ "- Returns the greatest value in the list.", extended = "Example:\n"
		+ " > SELECT _FUNC_(2, 5, 12, 3) FROM src;\n 12")
@HivePdkUnitTests(setup = "create table dual_data (i int); "
		+ "insert overwrite table dual_data select 1 from dual limit 1;", cleanup = "drop table if exists dual_data;", cases = {
		@HivePdkUnitTest(query = "SELECT nexr_greatest(2, 5, 12, 3) " + "FROM dual_data;", result = "12"),
		@HivePdkUnitTest(query = "SELECT nexr_greatest('2', '5', '12', '3') " + "FROM dual_data;", result = "5"),
		@HivePdkUnitTest(query = "SELECT nexr_greatest('apples', 'oranges', 'bananas') " + "FROM dual_data;", result = "oranges") })
		...
		for (int i = 0; i < arguments.length; i++) {
			if (!returnOIResolver.update(arguments[i])) {
				throw new UDFArgumentTypeException(i, "The value of return should have the same type: \""
						+ returnOIResolver.get().getTypeName() + "\" is expected but \"" + arguments[i].getTypeName()
						+ "\" is found");
			}
		}
