...
package com.naltel.hadoop.hive.genericUDF.examples;
 ...
 * string is the string to search. substring is the substring to search for in
 * string. start_position is the position in string where the search will start.
 * This argument is optional. If omitted, it defaults to 1. The first position
 * in the string is 1. If the start_position is negative, the function counts
 * back start_position number of characters from the end of string and then
 * searches towards the beginning of string. nth_appearance is the nth
 * appearance of substring. This is optional. If omitted, it defaults to 1.
 * 
 */
@Description(name = "instr", value = "_FUNC_(string, substring, [start_position, [nth_appearance]]) "
		+ "- Returns the index of the first occurance of substr in str", extended = "Example:\n"
		+ " > SELECT _FUNC_('Facebook', 'boo') FROM src LIMIT 1;\n" + " 5\n")
@HivePdkUnitTests(setup = "", cleanup = "", cases = {
		@HivePdkUnitTest(query = "SELECT nexr_instr('Tech on the net', 'e') FROM onerow;", result = "2"),
		@HivePdkUnitTest(query = "SELECT nexr_instr('Tech on the net', 'e', 1, 1) FROM onerow;", result = "2"),
		@HivePdkUnitTest(query = "SELECT nexr_instr('Tech on the net', 'e', 1, 2) FROM onerow;", result = "11"),
		@HivePdkUnitTest(query = "SELECT nexr_instr('Tech on the net', 'e', 1, 3) FROM onerow;", result = "14"),
		@HivePdkUnitTest(query = "SELECT nexr_instr('Tech on the net', 'e', -5, 1) FROM onerow;", result = "11") })
